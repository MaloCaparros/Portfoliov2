...
interface ProductProps extends Omit<Project, 'id'> {
  reversed?: boolean;
}

function Product({ name, link, description, image, reversed }: ProductProps) {
  return (
    <div
    ...
    >
      <div className="flex flex-col justify-center items-center p-6 flex-1 md:items-start">
        ...
        <Button label="Voir le Projet" href={link} color="white" border="black" />
      </div>
      {image && (
